...
cmd_sys_err_t cmd_impl_HELLO(const cmd_sys_cmd_t *cmd){

    static uint8_t input_message[32] = { 0 };

    // Read the message from the input stream
    uint32_t input_message_len = MIN((sizeof(input_message) - 1), (cmd->header.data_len));
    uint32_t bytes_read = io_stream_read(cmd->input, input_message, input_message_len, pdMS_TO_TICKS(CMD_SYS_INPUT_READ_TIMEOUT_MS), NULL);
    if (bytes_read != input_message_len) return CMD_SYS_ERR_READ_TIMEOUT;

    // Create the response message
    char hello_string[6] = "Hello ";
    char response_message[40];

    //copy hello into response
    strcpy(response_message, hello_string);

    //concat hello and input
    strcat(response_message, (char*)input_message);

    //get length of response message
    uint32_t response_message_len = strlen(response_message);

    // Write the response message
    cmd_sys_err_t err = CMD_SYS_SUCCESS;

    err = cmd_sys_begin_response(cmd, CMD_SYS_RESP_CODE_SUCCESS, response_message_len);
    if (err != CMD_SYS_SUCCESS) return err;

    uint32_t bytes_written = io_stream_write(cmd->output, (uint8_t*)response_message, response_message_len, pdMS_TO_TICKS(CMD_SYS_OUTPUT_WRITE_TIMEOUT_MS), NULL);
    if (bytes_written != response_message_len) return CMD_SYS_ERR_WRITE_TIMEOUT;
